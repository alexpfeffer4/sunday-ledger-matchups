...
export function PositionLedgerRow({ row }: { row: PositionLedgerItem }) {
  ...
  return (
    <li
      ...
      className={`position-ledger-item bg-surface rounded-lg border p-4 ${row.side === "SELF" ? "border-boundary border-l-registry border-l-4" : "border-boundary border-l-copper border-l-4"}`}
      data-position-id={row.id}
    >
      <div className="position-ledger-top flex min-w-0 items-start justify-between gap-3">
