...
import { fetchApiSportsBoxScore, type StatisticsUsage } from "@/adapters/providers/api-sports/client";
...
import { resultPlayerMappingSchema, type PlayerObservation } from "@/application/providers/player-results";

const contextSchema = z.object({
  externalEventId: z.string().min(1), sourceEventId: z.string().min(1),
  gameDate: z.iso.date(), final: z.literal(true),
  mappings: z.array(resultPlayerMappingSchema.extend({ pfrPlayerId: z.string().nullable() })).max(96),
});
const jobsSchema = z.object({
  status: z.enum(["DISABLED", "BACKOFF", "BUDGET", "IDLE", "CLAIMED"]),
  jobs: z.array(z.object({ leaseId: z.uuid(), context: contextSchema })).max(16),
});
...
export async function processPlayerResults() {
  ...
  let attempted = 0; let reconciled = 0; let failures = 0;
  const apiClaim = process.env.API_SPORTS_NFL_KEY
  ...
  for (let index = 0; index < jobs.jobs.length; index += 4) {
    await Promise.all(jobs.jobs.slice(index, index + 4).map(async (job) => {
      attempted++;
      let observations: PlayerObservation[] | null = null;
      let usage: StatisticsUsage = { remaining: null, rateLimit: null, retryAfterSeconds: null };
      try {
        const fetched = await fetchApiSportsBoxScore(job.context.sourceEventId, (value) => { usage = value; });
        observations = normalizeApiSportsPlayerResults(fetched.payload, {
          ...job.context, fetchedAt: fetched.fetchedAt,
      ...
      const completed = await admin.rpc("complete_player_result_request", {
        p_request_id: job.leaseId, p_observations: observations,
        p_remaining: usage.remaining, p_rate_limit: usage.rateLimit,
  ...
  if (reconciliation.jobs.length > 0) {
    ...
    let observations: PlayerObservation[] | null = null;
    try {
      const seasons = new Set(reconciliation.jobs.map((job) => Number(job.context.sourceEventId.slice(0, 4))));
      ...
      observations = reconciliation.jobs.flatMap((job) => normalizeNflversePlayerResults({
        ...job.context, ...files,
        sourceUpdatedAt: files.statsSourceUpdatedAt < files.snapsSourceUpdatedAt ? files.statsSourceUpdatedAt : files.snapsSourceUpdatedAt,
        statsComplete: true, snapsComplete: true,
      }));
      reconciled = reconciliation.jobs.length;
    } catch { failures++; }
    const completed = await admin.rpc("complete_nflverse_reconciliation", { p_lease_id: lease, p_observations: observations });
    if (completed.error) failures++;
  }
  return { status: failures ? "PARTIAL" : attempted || reconciled ? "PROCESSED" : "IDLE", attempted, reconciled };
}
