...
export function normalizeApiSportsPlayerResults(
  raw: unknown,
  context: {
    ...
    final: boolean;
    completeBoxScore: boolean;
    mappings: readonly ResultPlayerMapping[];
  },
): PlayerObservation[] {
  const parsed = responseSchema.parse(raw);
  if (String(parsed.parameters.id) !== context.sourceEventId)
    throw new Error("RESULT_EVENT_MISMATCH");
  return context.mappings.map((mapping) => {
    const rows = parsed.response.flatMap((team) =>
      team.players
    ...
    if (
      rows.length > 1 ||
      (rows[0] && rows[0].team !== (mapping.sourceTeam ?? mapping.team))
    )
    ...
    const groups =
      row?.groups.filter((group) => group.name.toLowerCase() === groupName) ??
      [];
    if (groups.length > 1) throw new Error("AMBIGUOUS_STATISTIC_GROUP");
    const values =
      groups[0]?.statistics.filter(
        (stat) => stat.name.toLowerCase() === "yards",
      ) ?? [];
    ...
    const offense =
      row?.groups.some(
        (group) =>
          ["passing", "rushing", "receiving"].includes(
    ...
    const evidence = {
      ...
      participationSourceUpdatedAt: context.sourceUpdatedAt,
