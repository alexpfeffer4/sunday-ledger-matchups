...
const marketsSchema = z.object({
  eligible: z.tuple([z.literal("MONEYLINE"), z.literal("SPREAD"), z.literal("TOTAL")]),
...
export type CardRules = Pick<PersistedSeasonRuleset, "card" | "concentration" | "markets">;
...
export function resolveSeasonCardRules(
  ...
  if (
    ...
    canonical.productBibleVersion !== (canonical.version === "1.2" ? "3.1" : "3.0")
  ) return { supported: false, message: unavailableCardRulesMessage };
  ...
  const rules = (canonical.version === "1.0"
    ? historicalCardRulesSchema : atomicCardRulesSchema).safeParse(raw);
