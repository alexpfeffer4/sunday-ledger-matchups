...
export const playerObservationSchema = z.object({
  ...
  if (observation.complete && observation.value === null) {
    context.addIssue({ code: "custom", message: "Complete statistics require a value." });
  }
  if (observation.participationComplete === (observation.participation === "UNKNOWN")) {
    context.addIssue({ code: "custom", message: "Participation requires explicit evidence." });
  }
  if (observation.sourceUpdatedAt !== null && Date.parse(observation.sourceUpdatedAt) > Date.parse(observation.fetchedAt)) {
    context.addIssue({ code: "custom", message: "Source evidence cannot be newer than its fetch." });
  }
});
...
export const resultPlayerMappingSchema = z.object({
  subjectId: z.uuid(), externalPlayerId: z.string().min(1),
  team: z.string().min(1), statistic: playerStatisticSchema,
...
export function nextPlayerResultAttempt(finalObservedAt: Date, attempts: number): Date | null {
  const minutes = PLAYER_RESULT_RETRY_MINUTES[attempts];
  return minutes === undefined ? null : new Date(finalObservedAt.getTime() + minutes * 60_000);
}
