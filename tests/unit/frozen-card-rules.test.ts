  ...
  return {
    ...
    canonicalJson: {
      ...
      card:
        version === "1.0"
          ? Object.fromEntries(
              Object.entries(canonical.card).filter(
                ([key]) =>
                  ![
                    "carryoverCredits",
                    "acceptanceUnit",
      ...
      concentration:
        version === "1.0"
          ? Object.fromEntries(
              Object.entries(canonical.concentration).filter(
                ([key]) => key !== "status",
              ),
            )
...
describe.each(["LIVE", "SIMULATION"] as const)(
  "%s frozen card compatibility",
  (mode) => {
    it.each(["1.0", "1.1", "1.2"] as const)(
      "uses V%s limits without changing its snapshot",
      (version) => {
        ...
        const cases = [
          ...
          {
            picks: Array.from({ length: 20 }, (_, i) => pick(String(i), 50)),
            accepted: true,
          },
          {
            picks: Array.from({ length: 21 }, (_, i) => pick(String(i), 50)),
            accepted: false,
          },
...
it("fails closed on unknown, missing, changed, extra, unfrozen and mismatched context", () => {
  ...
  const invalid = [
    ...
    { ...valid, canonicalJson: { ...valid.canonicalJson, version: "1.1" } },
    {
      ...
      canonicalJson: {
        ...valid.canonicalJson,
        card: { ...valid.canonicalJson.card, maximumPositions: 21 },
      },
    },
    {
      ...valid,
      canonicalJson: {
        ...valid.canonicalJson,
        card: { ...valid.canonicalJson.card, minimumStakeCredits: "50" },
      },
    },
    {
      ...valid,
      canonicalJson: {
        ...valid.canonicalJson,
        card: { ...valid.canonicalJson.card, newConstraint: true },
      },
    },
    {
      ...valid,
      canonicalJson: {
        ...valid.canonicalJson,
        concentration: {
          ...valid.canonicalJson.concentration,
          heavyFavoriteThresholdAmerican: -201,
        },
      },
    },
    {
  ...
  for (const snapshot of invalid)
    expect(resolveSeasonCardRules(snapshot, "LIVE").supported).toBe(false);
});
