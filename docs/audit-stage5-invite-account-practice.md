...
- Email callback GET immediately consumed credentials, so app-side prefetch could use a link. GET/HEAD now lead to a branded confirmation page without verification. Only an explicit same-origin POST exchanges the credential. Type validation, PKCE verifier checking, no-store responses, no-referrer policy, cookie forwarding and safe internal redirects remain. Auth outages are distinguished from invalid/expired links.
- Verified profile failure now has an inline retry that keeps the session and exact destination. Returning to Create account while signed in resumes setup. The chosen username survives recoverable action errors. A setup retry that receives Auth's `same_password` result may continue because the existing password already matches; other failures still block completion.
