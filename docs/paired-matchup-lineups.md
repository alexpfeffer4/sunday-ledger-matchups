...
- The scoreboard compacts below the measured league header during scrolling. Member names stay beside their scores. Focused controls stay available; short-height layouts use normal flow. Completed games can be collapsed for both sides together, with per-game returns still visible. Stable event keys retain the user's choice through an RSC refresh; a full reload starts expanded.
...
- The matchup selector browses authorized active-week pairings. The week selector links previous viewer results to their existing History entries; it does not pretend the current-week query contains historical lineups. Planned weeks are not offered as completed results.
