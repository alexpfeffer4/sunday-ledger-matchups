-- Owner-approved September 14: disclose only card-wide unsettled count and stake
-- after common lock. Keep event-level reveal, membership and rehearsal guards.
...
begin
  ...
  return jsonb_build_object('weekId', v_week.id, 'cards', coalesce((
    ...
    cross join lateral (
      ...
      from private.position_receipts receipt
      cross join lateral (
        -- Every settlement version has a terminal outcome. Corrections remain
        -- settled, so their append-only history must never multiply the totals.
      ...
      where receipt.card_id = card.id and receipt.week_id = v_week.id
        and v_now >= v_week.common_lock_at
    ) totals
