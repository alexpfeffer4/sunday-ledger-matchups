...
begin
  ...
  return next throws_ok(format('update private.season_ruleset_snapshots set canonical_json=%L::jsonb where id=%L::uuid',
    jsonb_set(v_original,'{card,weeklyAllocationCredits}','2000'),v_snapshot),'55000','A frozen season ruleset snapshot is immutable.','the frozen snapshot guard is restored after fixture setup');
end;
$$;
